...
class OpenAIInterface:
    ...
    def getCompletion(openaicfg, prompt, dynamic_retry):
        ...
        try:
            ...
            if openaicfg.model.startswith('ft:') or \
            (openaicfg.model.startswith(OpenAIInterface.CHAT_GPT_MODEL_NAME) and 'instruct' not in openaicfg.model):
                assert len(prompt) == 1, "Chat only supports one prompt"
                response = client.chat.completions.create(model=openaicfg.model,
                messages=[
                    {
                        "role": "system",
                        "content": "A math teacher chatbot that helps you find the answer without telling you the answer directly."
                    },
                    {
                        "role": "user",
                        "content": prompt[0]
                ...
                response = client.completions.create(model=openaicfg.model,
                prompt=prompt,
                temperature=openaicfg.temperature,
                max_tokens=openaicfg.max_tokens,
                top_p=openaicfg.top_p,
